...
def build_app(limit: int = 2, window_seconds: int = 60, buckets: list[str] | None = None):
    ...
    for bucket in buckets or ["test"]:
        ...
        @app.get(f"/{bucket}-public", dependencies=[Depends(rate_limit(bucket, limit, window_seconds))])
        async def limited():
            return {"ok": True}

        @app.get(f"/{bucket}-private", dependencies=[Depends(rate_limit(bucket, limit, window_seconds, private=True))])
...
def test_private_api_rate_limit_multiple_ips_same_user():
    app = build_app(limit=1)
...
def test_private_api_rate_limit_multiple_users_same_ip():
    app = build_app(limit=1)
...
def test_private_api_rate_limit_auth_unauth_mix():
    app = build_app(limit=1)
    client = build_client(app, TEST_IP)
